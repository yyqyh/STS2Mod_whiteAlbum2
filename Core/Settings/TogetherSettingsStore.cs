...
internal static class TogetherSettingsStore
{
    ...
    /// <summary>改一个字段并立刻落盘（决斗解绑关共生体时用）。</summary>
    public static void Update(Action<TogetherSettings> change)
    {
        Initialize();

        var store = RitsuLibFramework.GetDataStore(Const.ModId);
        store.Modify<TogetherSettings>(DataKey, settings => change(settings));
        store.Save(DataKey);
    }
